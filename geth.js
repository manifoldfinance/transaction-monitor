...
if (!input) {
  console.error(
    'Input log file required. Use "--input" to tell the program which file to process.',
  );
  process.exit(1);
}
...
const outStream = new Writable({
  write(chunk, encoding, callback) {
    ...
    let matches = lines.match(
      /\[([0-9]{2}-[0-9]{2}\|[0-9]{2}:[0-9]{2}:[0-9]{2})\]\s+Transaction pool status report\s+executable=(\d+)\s+queued=(\d+)\s+stales=(\d+)/g,
    );
    if (matches) {
      // there should be more than one matches, now parse each match to get the details
      for (let i = 0; i < matches.length; i++) {
        let one = matches[i].match(
          /\[([0-9]{2}-[0-9]{2}\|[0-9]{2}:[0-9]{2}:[0-9]{2})\]\s+Transaction pool status report\s+executable=(\d+)\s+queued=(\d+)\s+stales=(\d+)/,
        );
        ...
        console.log(
          'Pool at %s: executable - %s, \tnon-executable - %s, \tstale - %s',
          one[1],
          one[2],
          one[3],
          one[4],
    ...
    matches = lines.match(
      /\[([0-9]{2}-[0-9]{2}\|[0-9]{2}:[0-9]{2}:[0-9]{2})\]\s+(Inserted new block|Commit new mining work).+txs=(\d+)/g,
    );
    if (matches) {
      for (let i = 0; i < matches.length; i++) {
        let one = matches[i].match(
          /\[([0-9]{2}-[0-9]{2}\|[0-9]{2}:[0-9]{2}:[0-9]{2})\]\s+(Inserted new block|Commit new mining work).+txs=(\d+)/,
        );
        txtimes.push(one[1]);
